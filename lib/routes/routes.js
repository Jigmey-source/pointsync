...
router.post(routes.followUser, async function (req, res) {
    ...
    console.log(req.body);
    try {
        const newfollower = await user.findOne({ userId: req.body.userId });
        var increment = { $inc: { 'followers': 1 } };
        user.findOneAndUpdate(newfollower, increment, function (err, doc) {
            if (err) {
        ...
        const entry = await user.findOne({ userId: req.body.id });
        entry.following.push(newfollowing);
        await entry.save();
