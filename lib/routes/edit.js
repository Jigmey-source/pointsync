...
router.post(routes.updatestoredescription, async function (req, res) {
    ...
    // );
    try {
        updatedescriptioninstore(req.body.link, req.body.description);
        updatedescriptioninrecord(req.body.link, req.body.description);
        res.json('updated description in store and record');
    } catch (e) {
        res.json(e);
    }
});

function updatedescriptioninstore(link, description) {
    stores.updateOne(
        { link: link },
        { $set: { description: description } },
        function (e, doc) {
            if (e) {
                console.log('Error uploading description in store: ' + e);
            } else {
                console.log('description updated in store');
            }
        }
    )
}

function updatedescriptioninrecord(link, description) {
    records.updateOne(
        { 'records.link': link },
        { $set: { 'records.$[el].description': description } },
        { arrayFilters: [{ 'el.link': link }] },
        function (e, doc) {
            if (e) {
                console.log('Error update description in records: ' + e);
            } else {
                console.log('description updated in records');
            }
        }
    )
}
