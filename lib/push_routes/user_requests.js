...
router.post(routes.followee, async function (req, res) {
    ...
    try {
        ...
        incrementfollowers(req.body.id);
        res.json({ success: true });
    } catch (e) {
        console.log('Error eror ereroeroeroere ererererere ' + e);
        res.json(e);
    }
});

function incrementfollowers(id) {
...
router.post(routes.unfollow, async function (req, res) {
    ...
    try {
        ...
        decrementfollowers(req.body.id);
...
function pullfollowing(id, userId) {
    user.updateOne(
        { userId: userId },
        { $set: { $pull: { 'following': { userId: id } } }, $inc: { 'totalfollowing': -1 } },
        function (e, doc) {
            if (e) {
...
function decrementfollowers(id) {
