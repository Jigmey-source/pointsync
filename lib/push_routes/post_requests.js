    ...
    user.updateOne(
        ...
        function (e, doc) {
            if (e) {
                ...
                console.log('Posts incremented successfully ' + doc);
            }
        }
    )
}
...
function publishtofeeds(id, follower, snap) {
    ...
    const userId = follower.map(follower => { return follower.userId });
    console.log('id id id id id id id id id');
    console.log(userId);
    const doc = { userId, snap };
    feeds.insertMany(doc).then(function () { 
